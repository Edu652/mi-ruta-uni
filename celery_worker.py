# Fichero: celery_worker.py (Corregido y Completo)
import os
...
def calculate_route_times(ruta_series_list, desde_ahora_check, now):
    try:
        ...
        if len(segmentos) == 1 and segmentos[0].get('Tipo_Horario') == 'Frecuencia':
            ...
            # Convertimos fechas a string para Celery
        ...
        for seg in segmentos:
            seg['Salida_dt_iso'] = seg['Salida_dt'].isoformat()
            seg['Llegada_dt_iso'] = seg['Llegada_dt'].isoformat()
...
def find_routes_task(self, origen, destino, dia_seleccionado, form_data, now_iso):
    ...
    # ... (Toda la lógica de búsqueda, filtrado y cálculo se mantiene igual) ...
